...
function createCustomIcon(numberProblems: number, size: number) {
    ...
    const svgIcon = `
        <svg width="${iconSize}" height="${iconSize}" viewBox="0 0 42 55" xmlns="http://www.w3.org/2000/svg">
            ...
            ${numberProblems > 0 ? `<text x="21" y="25" font-family="Arial" font-size="12" fill="${color}" text-anchor="middle" dy=".3em"></text>` : ''}
        </svg>
    `;
...
function MarkerWithSmoothTransition({ item }: { item: MapData }) {
    ...
    const icon = useMemo(() => createCustomIcon(item.number_of_problem, currentSize), [item.number_of_problem, currentSize]);
    
    useEffect(() => {
        const animateSize = () => {
            if (currentSize !== targetSize) {
                const newSize = currentSize + (targetSize > currentSize ? 1 : -1);
                setCurrentSize(newSize);
                if (markerRef.current) {
                    const newIcon = createCustomIcon(item.number_of_problem, newSize);
            ...
            <Popup>
                <div className="text-black rounded-lg">
                    ...
                    <p className={`text-sm ${
                        ...
                        Trạng thái: {item.status === "active" ? "Đang hoạt động" : "Ngừng hoạt động"}
                    </p>
                    <p className="text-sm">
                        Số lỗi:{" "}
