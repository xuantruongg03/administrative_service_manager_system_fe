...
function EditBusiness() {
    ...
    const [fileBusinessLicense, setFileBusinessLicense] = useState<File | null>(null);
    const [fileFirePreventionLicense, setFileFirePreventionLicense] = useState<File | null>(null);
    const [fileSecurityLicense, setFileSecurityLicense] = useState<File | null>(null);
    const [nameFirePreventionLicense, setNameFirePreventionLicense] = useState<string | null>(null);
    const [nameBusinessLicense, setNameBusinessLicense] = useState<string | null>(null);
    const [nameSecurityLicense, setNameSecurityLicense] = useState<string | null>(null);
    const { id } = useParams();
    ...
    const { uploadBusinessLicense, isPending: isPendingUploadBusinessLicense } = useUploadBusinessLicense();
    const { uploadFirePreventionLicense, isPending: isPendingUploadFirePreventionLicense } = useUploadFirePreventionLicense();
    const { uploadSecurityLicense, isPending: isPendingUploadSecurityLicense } = useUploadSecurityLicense();
    const { removeLicense, isPending: isPendingRemoveLicense } = useRemoveLicense();

    //Functions
    const handleUploadBusinessLicense = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleUploadFirePreventionLicense = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (file) {
    ...
    const handleUploadSecurityLicense = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (file) {
            setFileSecurityLicense(file);
            uploadSecurityLicense({ file, id: id as string }).then(() => {
                toast.success("Tải lên giấy chứng nhận đủ điều kiện về ANTT thành công");
            });
        }
    }
    ...
    useEffect(() => {
        if (dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Fire Prevention License')) {
            setNameFirePreventionLicense(dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Fire Prevention License')?.name);
        }
        if (dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Business License')) {
            setNameBusinessLicense(dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Business License')?.name);
        }
        if (dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Security License')) {
            setNameSecurityLicense(dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Security License')?.name);
        }
            ...
            <div className="flex flex-col gap-8 p-6 bg-gray-100 rounded-lg shadow-md">
                <form onSubmit={onSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm">
                        <div className="space-y-8">
                            <div>
                                ...
                                <input type="hidden" {...register("legal_representative.id")} value={dataBusiness?.data?.legal_representative.id} />
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            ...
                                            className={`w-full p-3 text-sm ${errors.legal_representative?.citizen_id ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.legal_representative.citizen_id}
                                            {...register("legal_representative.citizen_id", { 
                                                required: "Số CCCD là bắt buộc",
                                                pattern: {
                                                    value: REGEX.ID_CARD,
                                                    message: "Số CCCD phải có 8 hoặc 12 chữ số"
                                                }
                                            })}
                                        />
                                        {errors.legal_representative?.citizen_id && (
                                            <p className="text-red-500 text-sm mt-1">{errors.legal_representative.citizen_id.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            ...
                                            className={`w-full p-3 text-sm ${errors.legal_representative?.name ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.legal_representative.name}
                                            {...register("legal_representative.name", { 
                                        ...
                                        {errors.legal_representative?.name && (
                                            <p className="text-red-500 text-sm mt-1">{errors.legal_representative.name.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            ...
                                            type="date"
                                            className={`w-full p-3 text-sm ${errors.legal_representative?.birth_date ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.legal_representative.birth_date}
                                            {...register("legal_representative.birth_date", { 
                                                required: "Ngày sinh là bắt buộc",
                                                validate: value => {
                                                    const birthDate = new Date(value);
                                                    const today = new Date();
                                                    const age = today.getFullYear() - birthDate.getFullYear();
                                                    return age >= 18 || "Người đại diện phải từ 18 tuổi trở lên";
                                                }
                                            })}
                                        />
                                        {errors.legal_representative?.birth_date && (
                                            <p className="text-red-500 text-sm mt-1">{errors.legal_representative.birth_date.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                        ...
                                        <select
                                            ...
                                            className={`w-full p-3 border rounded-md ${errors.legal_representative?.gender ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.legal_representative.gender}
                                            {...register("legal_representative.gender", { 
                                        ...
                                        {errors.legal_representative?.gender && (
                                            <p className="text-red-500 text-sm mt-1">{errors.legal_representative.gender.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            ...
                                            className={`w-full p-3 text-sm ${errors.legal_representative?.nationality ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.legal_representative.nationality}
                                            {...register("legal_representative.nationality", { 
                                                required: "Quốc tịch là bắt buộc"
                                            })}
                                        />
                                        {errors.legal_representative?.nationality && (
                                            <p className="text-red-500 text-sm mt-1">{errors.legal_representative.nationality.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                    ...
                                    <div className="col-span-2">
                                        ...
                                        <InputText
                                            ...
                                            className={`w-full p-3 text-sm ${errors.legal_representative?.hometown ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.legal_representative.hometown}
                                            {...register("legal_representative.hometown", { 
                                                required: "Quê quán là bắt buộc"
                                            })}
                                        />
                                        {errors.legal_representative?.hometown && (
                                            <p className="text-red-500 text-sm mt-1">{errors.legal_representative.hometown.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-2">
                                        ...
                                        <InputText
                                            ...
                                            className={`w-full p-3 text-sm ${errors.legal_representative?.current_address ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.legal_representative.current_address}
                                            {...register("legal_representative.current_address", { 
                                                required: "Địa chỉ hiện tại là bắt buộc"
                                            })}
                                        />
                                        {errors.legal_representative?.current_address && (
                                            <p className="text-red-500 text-sm mt-1">{errors.legal_representative.current_address.message}</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm">
                        <div className="space-y-8">
                            <div>
                                ...
                                <input type="hidden" {...register("owner.id")} value={dataBusiness?.data?.owner.id} />
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            id="owner.citizen_id"
                                            className={`w-full p-3 text-sm ${errors.owner?.citizen_id ? 'border-red-500' : ''}`}
                                        ...
                                        {errors.owner?.citizen_id && (
                                            <p className="text-red-500 text-sm mt-1">{errors.owner.citizen_id.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            id="owner.name"
                                            className={`w-full p-3 text-sm ${errors.owner?.name ? 'border-red-500' : ''}`}
                                    ...
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            id="owner.birth_date"
                                            type="date"
                                            className={`w-full p-3 text-sm ${errors.owner?.birth_date ? 'border-red-500' : ''}`}
                                            ...
                                            {...register("owner.birth_date", { 
                                                ...
                                                validate: value => {
                                                    ...
                                                    const today = new Date();
                                                    const age = today.getFullYear() - birthDate.getFullYear();
                                                    return age >= 18 || "Chủ sở hữu phải từ 18 tuổi trở lên";
                                                }
                                            })}
                                        />
                                        {errors.owner?.birth_date && (
                                            <p className="text-red-500 text-sm mt-1">{errors.owner.birth_date.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                        ...
                                        <select
                                            id="owner.gender"
                                            className={`w-full p-3 border rounded-md ${errors.owner?.gender ? 'border-red-500' : ''}`}
                                        ...
                                        {errors.owner?.gender && (
                                            <p className="text-red-500 text-sm mt-1">{errors.owner.gender.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                        ...
                                        <InputText
                                            ...
                                            className={`w-full p-3 text-sm ${errors.owner?.nationality ? 'border-red-500' : ''}`}
                                        ...
                                        {errors.owner?.nationality && (
                                            <p className="text-red-500 text-sm mt-1">{errors.owner.nationality.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-1">
                                    ...
                                    <div className="col-span-2">
                                        ...
                                        <InputText
                                            id="owner.hometown"
                                            className={`w-full p-3 text-sm ${errors.owner?.hometown ? 'border-red-500' : ''}`}
                                        ...
                                        {errors.owner?.hometown && (
                                            <p className="text-red-500 text-sm mt-1">{errors.owner.hometown.message}</p>
                                        )}
                                    </div>
                                    <div className="col-span-2">
                                        ...
                                        <InputText
                                            ...
                                            className={`w-full p-3 text-sm ${errors.owner?.current_address ? 'border-red-500' : ''}`}
                                            defaultValue={dataBusiness?.data?.owner.current_address}
                                            {...register("owner.current_address", { 
                                                required: "Địa chỉ hiện tại là bắt buộc"
                                            })}
                                        />
                                        {errors.owner?.current_address && (
                                            <p className="text-red-500 text-sm mt-1">{errors.owner.current_address.message}</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm lg:col-span-2">
                        ...
                        <input type="hidden" {...register("id")} value={dataBusiness?.data?.id} />
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div className="md:col-span-1 col-span-1">
                                <label htmlFor="name_vietnamese" className="block text-sm font-medium text-gray-700 mb-1">
                                    Tên tiếng việt *
                                </label>
                                <InputText
                                    id="name_vietnamese"
                                    className={`w-full p-3 text-sm ${errors.name_vietnamese ? 'border-red-500' : ''}`}
                                    defaultValue={dataBusiness?.data?.name_vietnamese}
                                ...
                                {errors.name_vietnamese && (
                                    <p className="text-red-500 text-sm mt-1">{errors.name_vietnamese.message}</p>
                                )}
                            </div>
                            <div className="md:col-span-1 col-span-1">
                                <label htmlFor="name_english" className="block text-sm font-medium text-gray-700 mb-1">
                                    Tên tiếng anh
                                </label>
                                <InputText
                                    id="name_english"
                            ...
                            <div className="md:col-span-1 col-span-1">
                                <label htmlFor="name_acronym" className="block text-sm font-medium text-gray-700 mb-1">
                                    Tên viết tắt
                                </label>
                                <InputText
                                    id="name_acronym"
                            ...
                            <div className="md:col-span-1 col-span-1">
                                <label htmlFor="chartered_capital" className="block text-sm font-medium text-gray-700 mb-1">
                                    Vốn điều lệ *
                                </label>
                                <InputText
                                    type="number"
                                    id="chartered_capital"
                                    className={`w-full p-3 text-sm ${errors.chartered_capital ? 'border-red-500' : ''}`}
                                    defaultValue={dataBusiness?.data?.chartered_capital}
                                ...
                                {errors.chartered_capital && (
                                    <p className="text-red-500 text-sm mt-1">{errors.chartered_capital.message}</p>
                                )}
                            </div>
                            <div className="md:col-span-3 col-span-2">
                                ...
                                <InputText
                                    id="address"
                                    className={`w-full p-3 text-sm ${errors.address ? 'border-red-500' : ''}`}
                            ...
                            <div className="md:col-span-1 col-span-1">
                                ...
                                <InputText
                                    type="tel"
                                    id="phone"
                                    className={`w-full p-3 text-sm ${errors.phone ? 'border-red-500' : ''}`}
                            ...
                            <div className="md:col-span-1 col-span-1">
                                ...
                                <InputText
                                    type="email"
                                    id="email"
                                    className={`w-full p-3 text-sm ${errors.email ? 'border-red-500' : ''}`}
                            ...
                            <div className="md:col-span-1 col-span-1">
                                ...
                                <InputText
                                    id="website"
                                    className={`w-full p-3 text-sm ${errors.website ? 'border-red-500' : ''}`}
                            ...
                            <div className="md:col-span-1 col-span-1">
                                ...
                                <InputText
                                    type="date"
                                    id="created_at"
                                    className={`w-full p-3 text-sm ${errors.created_at ? 'border-red-500' : ''}`}
                                ...
                                />
                                {errors.created_at && (
                                    <p className="text-red-500 text-sm mt-1">{errors.created_at.message}</p>
                                )}
                            </div>
                            <div className="md:col-span-1 col-span-1">
                                <label htmlFor="type_of_organization" className="block text-sm font-medium text-gray-700 mb-1">
                                    Loại hình doanh nghiệp *
                                </label>
                                <select
                                    id="type_of_organization"
                                    className={`w-full p-4 border rounded-md ${errors.type_of_organization ? 'border-red-500' : ''}`}
                                    defaultValue={dataBusiness?.data?.type_of_organization}
                                    {...register("type_of_organization", { 
                                        required: "Loại hình doanh nghiệp là bắt buộc"
                                    })}
                                >
                                ...
                                {errors.type_of_organization && (
                                    <p className="text-red-500 text-sm mt-1">{errors.type_of_organization.message}</p>
                                )}
                            </div>
                            <div className="md:col-span-1 col-span-1">
                            ...
                            <div className="md:col-span-1 col-span-2">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    />
                                    {fileBusinessLicense || nameBusinessLicense ? (
                                        <div className="flex items-center gap-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-md w-full">
                                            <span className="text-sm truncate hover:text-clip" title={fileBusinessLicense?.name || nameBusinessLicense || ''}>
                                                {fileBusinessLicense?.name || nameBusinessLicense}
                                            </span>
                                            <div className="flex items-center flex-shrink-0">
                                                {isPendingRemoveLicense ? (
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setFileBusinessLicense(null);
                                                            handleRemoveLicense(dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Business License')?.id, 'Business License');
                                                        }}
                                                    >
                                                        ×
                                                    </button>
                                        ...
                                        >
                                            {isPendingUploadBusinessLicense ? (
                                                ...
                                                <>
                                                    ...
                                                    <span className="text-sm">Tải lên giấy đăng kí kinh doanh</span>
                                                </>
                                            )}
                                        </label>
                                    )}
                            ...
                            <div className="md:col-span-1 col-span-2">
                                ...
                                <div className="flex items-center gap-4">
                                    <input
                                        ...
                                        className="hidden"
                                        accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
                                        onChange={handleUploadFirePreventionLicense}
                                    />
                                    {fileFirePreventionLicense || nameFirePreventionLicense ? (
                                        <div className="flex items-center gap-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-md w-full">
                                            <span className="text-sm truncate hover:text-clip" title={fileFirePreventionLicense?.name || nameFirePreventionLicense || ''}>
                                                {fileFirePreventionLicense?.name || nameFirePreventionLicense}
                                            </span>
                                            <div className="flex items-center flex-shrink-0">
                                                {isPendingRemoveLicense ? (
                                                    <LoadingMini />
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setFileFirePreventionLicense(null);
                                                            handleRemoveLicense(dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Fire Prevention License')?.id, 'Fire Prevention License');
                                                        }}
                                                    >
                                                        ×
                                                    </button>
                                        ...
                                        >
                                            {isPendingUploadFirePreventionLicense ? (
                                                ...
                                                <>
                                                    <FiUpload className="size-4 flex-shrink-0" />
                                                    <span className="text-sm">Tải lên giấy phép PCCC</span>
                                                </>
                                            )}
                                        </label>
                                    )}
                            ...
                            <div className="md:col-span-1 col-span-2">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    />
                                    {fileSecurityLicense || nameSecurityLicense ? (
                                        <div className="flex items-center gap-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-md w-full">
                                            <span className="text-sm truncate hover:text-clip" title={fileSecurityLicense?.name || nameSecurityLicense || ''}>
                                                {fileSecurityLicense?.name || nameSecurityLicense}
                                            </span>
                                            <div className="flex items-center flex-shrink-0">
                                                {isPendingRemoveLicense ? (
                                                    <LoadingMini />
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setFileSecurityLicense(null);
                                                            handleRemoveLicense(dataBusiness?.data?.licenses?.find((license: LicenseDataApi) => license.type === 'Security License')?.id, 'Security License');
                                                        }}
                                                    >
                                                        ×
                                                    </button>
                                        ...
                                        >
                                            {isPendingUploadSecurityLicense ? (
                                                ...
                                                <>
                                                    <FiUpload className="size-4 flex-shrink-0" />
                                                    <span className="text-sm">Tải lên giấy chứng nhận đủ đk về ANTT</span>
                                                </>
                                            )}
                                        </label>
                                    )}
